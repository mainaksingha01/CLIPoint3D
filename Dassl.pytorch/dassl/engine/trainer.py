    ...
    def resume_model_if_exist(self, directory):
        ...
        for name in names:
            ...
            if osp.exists(path):
                ...
                loaded_checkpoints.append(name)
            else:
                print(f"Checkpoint not found, skipping: {path}")

        if not loaded_checkpoints:
            print("No checkpoints found, train from scratch")
            return 0
        
        print(f"Successfully loaded {len(loaded_checkpoints)}/{len(names)} checkpoints (will resume training)")
