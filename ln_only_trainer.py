...
class LayerNormOnlyTrainer(TrainerXU):
    ...
    def build_model(self):
        ...
        print("Building MODEL with LayerNorm-only fine-tuning")
        self.model = Model(cfg, classnames, clip_model, peft_method='lora', text_peft_method='ln_only')
        ...
        # Check if image_encoder has LayerNorm-only parameters - if so, we'll optimize only those
        has_ln_only_image_encoder = (hasattr(self.model, 'image_encoder') and 
                                    hasattr(self.model.image_encoder, 'get_ln_only_parameters') and
                                    len(self.model.image_encoder.get_ln_only_parameters()) > 0)
        
        if has_ln_only_image_encoder:
            name_to_update.append("image_encoder")
            print("Image encoder has LayerNorm-only parameters - will optimize only LayerNorm parameters, not the full encoder.")

        # Check if text_encoder has LayerNorm-only parameters - if so, we'll optimize only those
        has_ln_only_text_encoder = (hasattr(self.model, 'text_encoder') and 
                                   hasattr(self.model.text_encoder, 'get_ln_only_parameters') and
                                   len(self.model.text_encoder.get_ln_only_parameters()) > 0)
        
        if has_ln_only_text_encoder:
            name_to_update.append("text_encoder")
            print("Text encoder has LayerNorm-only parameters - will optimize only LayerNorm parameters, not the full encoder.")
        ...
        # Special handling: For LayerNorm-only image_encoder, freeze non-LayerNorm params to save memory
        if has_ln_only_image_encoder:
            ln_only_params = self.model.image_encoder.get_ln_only_parameters()
            print(f"Found {len(ln_only_params)} LayerNorm parameter tensors in image_encoder.")
            
            # Enable only LayerNorm parameters
            for param in ln_only_params:
                param.requires_grad_(True)
                
            # Explicitly freeze non-LayerNorm parameters in image_encoder
            for name, param in self.model.image_encoder.named_parameters():
                if not any(param is ln_param for ln_param in ln_only_params):
                    param.requires_grad_(False)
        ...
        for name in name_to_update:
            if hasattr(self.model, name):
                ...
                # Special case: for LayerNorm-only image_encoder, optimize only LayerNorm parameters
                if name == "image_encoder" and has_ln_only_image_encoder:
                    ln_only_params = module.get_ln_only_parameters()
                    self.optimizers[name] = build_optimizer(ln_only_params, cfg.OPTIM)
                    print(f"Created optimizer for image_encoder with {len(ln_only_params)} LayerNorm parameter tensors")
                # Special case: for LayerNorm-only text_encoder, optimize only LayerNorm parameters
                elif name == "text_encoder" and has_ln_only_text_encoder:
                    ln_only_params = module.get_ln_only_parameters()
                    self.optimizers[name] = build_optimizer(ln_only_params, cfg.OPTIM)
                    print(f"Created optimizer for text_encoder with {len(ln_only_params)} LayerNorm parameter tensors")
                else:
        ...
        # Print LayerNorm-only specific statistics if available
        if has_ln_only_image_encoder:
            ln_only_params = self.model.image_encoder.get_ln_only_parameters()
            ln_only_param_count = sum(p.numel() for p in ln_only_params)
            print(f"LayerNorm-only parameters in image_encoder: {ln_only_param_count:,} ({ln_only_param_count/total_params:.4f} of total)")
            frozen_encoder_params = sum(p.numel() for name, p in self.model.image_encoder.named_parameters() 
                                       if not any(p is ln_param for ln_param in ln_only_params))
            print(f"Frozen parameters in image_encoder: {frozen_encoder_params:,}")
            
        if has_ln_only_text_encoder:
            ln_only_params = self.model.text_encoder.get_ln_only_parameters()
            ln_only_param_count = sum(p.numel() for p in ln_only_params)
            print(f"LayerNorm-only parameters in text_encoder: {ln_only_param_count:,} ({ln_only_param_count/total_params:.4f} of total)")
