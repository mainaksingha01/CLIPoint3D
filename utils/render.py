...
# # Create 30 views with 12-degree steps (2π/30 = π/15 ≈ 12 degrees)
# _views = np.array([[
#     [i * (2 * np.pi / 30), 0, np.pi / 2],  # Rotating around azimuth
#     [-0.5, -0.5, TRANS]
# ] for i in range(30)])

# # Bias array matching the 30 views
# _views_bias = np.array([[
#     [0, np.pi / 9, 0],  # Adding slight upward tilt
#     [-0.5, 0, TRANS]
# ] for i in range(30)])

# # Generate views with fewer steps: 20 azimuth steps (18° each) and 10 elevation steps
# azimuths = np.linspace(0, 2 * np.pi, 20)  # 20 steps around
# elevations = np.linspace(-np.pi/2, np.pi/2, 10)  # 10 elevation steps, from -60° to 60°

# _views = []
# for azim in azimuths:
#     for elev in elevations:
#         _views.append([[azim, elev, np.pi/2], [-0.5, -0.5, TRANS]])
# _views = np.asarray(_views)

# # Simplified bias pattern - small upward tilt and slight shift
# _views_bias = np.tile(
